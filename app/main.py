...
def _materialize_ssh_key(settings) -> None:
    ...
    The AWS server's `.env` supplies the Bisk Farm SSH key as SSH_KEY_B64
    (no key file on disk); it is decoded and written to SSH_KEY_PATH here on
    startup. No-op when the key file already exists or B64 is unset.
    """
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    Initializes the Postgres connection pool and the MySQL/SSH analytics
    connector on startup, registers their readiness probes, and warms up the
    Cloudflare-API embedder — tearing them all down on shutdown.
    """
